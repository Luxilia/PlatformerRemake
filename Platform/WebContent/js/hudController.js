...
function checkChangesToHUD() {
    ...
    if(showingTips){
    	tipsTime += 1;
    	if(tipsTime > tipsLimit){
    	tipsTrackerSprite.material.opacity -= 0.01;
    	}
    	if(tipsTrackerSprite.material.opacity < 0){
    		hideTips();
    	}
    }
    checkTips();
...
function createTipsTracker(){
    ...
    tipsContext.font = 'Bold 35px Arial';

    tipsContext.fillStyle = 'red';
    tipsContext.fillText(tipsText, 0, 40);
    tipsContext.strokeStyle = 'black';
...
function resetTips(){
	for (var i = 0; i < levelTips.length; i++){
		levelTips[i].reset();
	}
}
