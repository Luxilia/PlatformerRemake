...
function generateParts(x, y, z, i){
	...
	if(i == 13){
		createFloor(x,y,z);
		createFallingRoofTrapLaserZ(x,y,z);
	}
	if(i == 14){
		createFloor(x,y,z);
	...
	if(i == 26){
		createFloor(x,y,z);
		createFallingRoofTrapLaserX(x,y,z);
	}
	if(i == 27){
		createFloor(x,y,z);
		createExtraLife(x,y,z);
	}
	if(i == 28){
		createFloor(x,y,z);
		createHalfWall(x,y,z);
	}
//	if(i != 3 && y != 2 && i != 28){
//		createRoofTile(x,y,z);
//	}
...
function createFloor(x,y, z){
	...
	var roofPart = new Physijs.BoxMesh(new THREE.BoxGeometry(UNITSIZE,1,UNITSIZE), 
		...
		roofPart.position.set(UNITSIZE * x, (y)*WALLHEIGHT -0.5, UNITSIZE * z);
		scene.add(roofPart);
}
...
function generateRoofTopLevel(){
	for(var i = 0; i < mapW; i++){
		for(var j = 0; j < mapH; j++){
			createRoofTile(i, maps.length - 1, j);
		}
	}
}
...
function generateInvisibleWall(){
	var basicWall1 = new Physijs.BoxMesh(new THREE.BoxGeometry(UNITSIZE, WALLHEIGHT, UNITSIZE),
	        Physijs.createMaterial(new THREE.MeshBasicMaterial({
	                map: textureArray['floral'], opacity: 0.1
	        }), 0.0, 0.01), 0);
...
function createVerticalDoor(x, y, z){
	doorNumber += 1;
	var door = new Door(new THREE.Vector3(0,1,0), new THREE.Vector3(1,0,0), new THREE.Vector3(x*UNITSIZE, y*WALLHEIGHT + WALLHEIGHT / 2, z*UNITSIZE), new THREE.Vector3(UNITSIZE, WALLHEIGHT*6/8, UNITSIZE/4), doorNumber);
	doors.push(door);
}

function createJumpableDoor(x, y, z){
	doorNumber += 1;
	var door = new Door(new THREE.Vector3(0,1,0), new THREE.Vector3(0,0,1), new THREE.Vector3(x*UNITSIZE, y*WALLHEIGHT + WALLHEIGHT / 2, z*UNITSIZE), new THREE.Vector3(UNITSIZE/4, WALLHEIGHT*6/8, UNITSIZE), doorNumber);
	doors.push(door);
}
...
function createFallingRoofTrapLaserZ(x,y,z){
	...
	trigger.createCylinderMesh(new THREE.Vector3(0.1, 0.1, UNITSIZE), new THREE.Vector3(UNITSIZE*x, WALLHEIGHT*y + 1, UNITSIZE*z), null, Math.PI/2, 0, 0, 0, 0);
	fallingRooftraps.push(trap);
}

function createFallingRoofTrapLaserX(x,y,z){
	var trap = new Trap(new THREE.Vector3(0,1,0), 100, new THREE.Vector3(0,-4,0), 0);
	trap.createBoxMesh(new THREE.Vector3(UNITSIZE, 1, UNITSIZE), new THREE.Vector3(UNITSIZE*x, WALLHEIGHT*(y+1) - 1, UNITSIZE*z), null, 200, 0, 0.1);
	var traps = [];
	traps.push(trap);
	var trigger = new TrapTrigger(traps);
	trigger.createCylinderMesh(new THREE.Vector3(0.1, 0.1, UNITSIZE), new THREE.Vector3(UNITSIZE*x, WALLHEIGHT*y + 1, UNITSIZE*z), null, 0, Math.PI/2, 0, 0, 0);
	fallingRooftraps.push(trap);
}
...
function createHorizontalSpikeTrapZ(x,y,z){
	...
	trap.createBoxMesh(new THREE.Vector3(UNITSIZE, UNITSIZE, 1), new THREE.Vector3(UNITSIZE*x, WALLHEIGHT*y + WALLHEIGHT/3, UNITSIZE*z - UNITSIZE/3), null, 200, 0, 0.1);
	var traps = [];
	traps.push(trap);
	var trigger = new TrapTrigger(traps);
...
function createExtraLife(x,y,z){
	var life = new GatherItem("lives", 1, "life", true, false);
	var lifeMaterial = Physijs.createMaterial(new THREE.MeshBasicMaterial({map: textureArray['lifeBox']}),0,0);
	life.createBoxMesh(new THREE.Vector3(UNITSIZE/2, UNITSIZE/2, UNITSIZE/2), new THREE.Vector3(UNITSIZE*x, WALLHEIGHT*y + 2, UNITSIZE*z), lifeMaterial, 1);
}

function createHalfWall(x,y,z){
	var wallPart = new Physijs.BoxMesh(new THREE.BoxGeometry(UNITSIZE,WALLHEIGHT*4/8,UNITSIZE), 
			Physijs.createMaterial(new THREE.MeshBasicMaterial({
				map:wallTexture
			}),0, 0.1), 0);
	wallPart.position.set(UNITSIZE * x, y*WALLHEIGHT + WALLHEIGHT/4, UNITSIZE * z);
	scene.add(wallPart);
}
